...
export class Orchestrator {
  ...
  async addReceiver(receiver: ReceiverConfig) {
    ...
    await receiverManager.init();
  }

  async start() {
    await Promise.all(this.mqttManagers.map((m) => m.connect()));

    await Promise.all(this.receiverManagers.map((r) => r.init()));
  ...
  public static async run(options: OrchestratorOptions) {
    ...
    for await (const config of options.receivers) {
      await orchestrator.addReceiver(config);
    }
